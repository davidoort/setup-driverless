...
public:
  ...
  track::Line findCenterLine(std::vector<track::Cone> visibleCones) {
    ...
    for (int i=0; i<n_cones; i++) {
      ...
      ROS_INFO("%f", seen_cones_coords[i][0]);
    };
    ...
    float discretisation_y = y_max/100;
    float discretisation_x = x_max/100;

    ROS_INFO("%f", discretisation_x);

    // Fill matrix with classification
    ROS_INFO("Test");
    for (float i = 1; i < y_max; i += discretisation_y){     // loop through rows, omit first entry
      for (float j = 0; j < x_max; j += discretisation_x){ // loop through columns, omit first entry
        // {
        //     if (i == 1){
