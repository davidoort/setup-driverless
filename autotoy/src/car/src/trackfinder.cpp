...
public:
  ...
  car::Location::ConstPtr car_location;
  ...
  track::Line findCenterLine(std::vector<track::Cone> visibleCones) {
    ...
    if (n_cones > 0) {
      ...
      float x_max = *std::max_element(seen_cones_x, seen_cones_x+n_cones) - *std::min_element(seen_cones_x, seen_cones_x+n_cones);     
      float y_max = *std::max_element(seen_cones_y, seen_cones_y+n_cones) - *std::min_element(seen_cones_y, seen_cones_y+n_cones);
      ...
      float discretisation_y = y_max/10;
      float discretisation_x = x_max/10;



      // Get centerline

      for (float i = 1; i < y_max; i += discretisation_y){     // loop through rows, omit first entry
        for (float j = 1; j < x_max; j += discretisation_x){ // loop through columns, omit first entry
          // {
          //     if (i == 1){
      ...
      // Get car coordinates
      track::Point car_coords = car_location->location;
