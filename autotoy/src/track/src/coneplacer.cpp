...
// conesobj is reference to temporary holder for cone arrays || cones are changed by reference so no return values
{
  ...
  track::Point* leftpoint; // to store temporary point for left cones
  track::Point* rightpoint; // to store temporary point for right cones
...
bool server(track::ConePlacer::Request  &req, track::ConePlacer::Response &res)
{
  ...
  conePlacer(req.centreline, 1.5, *conesptr); // fill Cones objects in conesptr object
